...
class DOMTreeSerializer:
	...
	def _is_interactive_cached(self, node: EnhancedDOMTreeNode) -> bool:
		...
		if node.node_id not in self._clickable_cache:
			...
			# 🔁 ARIA override: some custom elements (x-*) only暴露 role，
			# 但 ClickableElementDetector 不一定懂，例如 role="option" 的圖層項目
			ax_role = node.ax_node.role.lower() if node.ax_node and node.ax_node.role else None
			if not result and ax_role in self.ARIA_INTERACTIVE_ROLES:
				result = True
	...
	def _build_attributes_string(node: EnhancedDOMTreeNode, include_attributes: list[str], text: str) -> str:
		...
		# 只有這些 role 的 selected 才有「目前被選取」的意義
		selectable_roles = {
			'option',        # listbox / combobox 的項目，包含 Layers panel 的項目
			'listitem',
			'row',
			'gridcell',
			'tab',
		...
		# --- DOM level selected / aria-selected 映射成統一的 selected=true/false ---
		dom_role = None
		if node.attributes and 'role' in node.attributes:
			dom_role = str(node.attributes['role']).strip().lower() or None

		effective_role = ax_role or dom_role
		roledescription = None
		if node.ax_node and node.ax_node.properties:
			for prop in node.ax_node.properties:
                # AX property 叫 roledescription，不是 DOM attribute
				if prop.name == 'roledescription' and prop.value:
					roledescription = str(prop.value).strip().lower()
					break

        # 這就是你原本想寫的 is_layer_option，但用的是 AX 資料結構
		is_layer_option = (
            effective_role == 'option'
            and roledescription == 'layers'
        )

        # 給 LLM 一個非常明確的 hint：這是一個 Layers panel 的 row
		if is_layer_option:
			attributes_to_include['layers-option'] = 'true'


		if node.attributes and effective_role in selectable_roles and 'selected' in include_attributes:
			# aria-selected="true/false" → selected=true/false
			if 'aria-selected' in node.attributes:
				raw = str(node.attributes['aria-selected']).strip().lower()
				if raw in ('true', 'false'):
					attributes_to_include['selected'] = raw

			# boolean selected="" (像 <x-sortable-item selected="">) → selected=true
			if 'selected' in node.attributes:
				raw = node.attributes.get('selected')
				raw_str = '' if raw is None else str(raw).strip()
				if not raw_str:
					attributes_to_include['selected'] = 'true'
				elif raw_str.lower() in ('true', 'false'):
					attributes_to_include['selected'] = raw_str.lower()

		# --- AX properties ---
		if node.ax_node and node.ax_node.properties:
			for prop in node.ax_node.properties:
				try:
					if prop.name not in include_attributes or prop.value is None:
						continue

					# ✅ 關鍵：如果是 StaticText 這種非 selectable role，就忽略 selected
					if prop.name == 'selected' and ax_role not in selectable_roles:
						# 例如 live region 裡的 StaticText "Text layer, 1 of 2, selected"
						# 只代表「播報過」，不是一個可操作的圖層
						continue
