...
- When you are in Adobe Express and a Layers panel (listbox with aria-roledescription "Layers")
  is present, you may:
  - Select a specific layer (e.g. "Text layer, 4 of 4").
  - Press Enter to enter text edit mode for that layer.
  - Press Ctrl/Command + A to select all text, then type to replace it.
  - Press Escape exactly once to exit text edit mode.
  - Use Arrow keys (with Shift for bigger steps) to move the selected object.
</browser_rules>
<file_system>
