...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if hasattr(llm, "provider") and llm.provider == 'browser-use':
			flash_mode = True
		...
		# Initialize state
		self.state = injected_agent_state or AgentState(judge_retries=judge_retries)
		...
		# Judgement Retry
		self.original_task = task # keep the original task, as the self.task variable is overwritten based on the agents usage.
	...
	# updated to return the JudgementResult class
	async def _judge_and_log(self) -> JudgementResult | None:
	...
	async def run(
		...
		try:
			...
			self.logger.info(f'🔄 Starting main execution loop with max {max_steps} steps...')
			...
			while step <= max_steps:
				...
				if is_done:
					...
					if self.settings.use_judge:
						# TODO: Return the judgment from this function below
						# TODO: If our agent state.judge_retries > 0 run that shit back
						# TODO: Pass the info from the judgement into the prompt / chat history (I think it is technically added on its own via this function call)
						# but using it more explicitly could be handy dandy
						judgement = await self._judge_and_log()

						if judgement and not judgement.verdict and self.state.judge_retries > 0: # was able to get judged but we failed the task
							# subtract judge retries
							self.logger.info(f"👨🏻‍⚖️JUDGEMENT INITIATED RESTART\n\nFailure determined: {judgement.failure_reason}\nRetrying: {self.state.judge_retries} times")
							self.state.judge_retries -= 1
							self.logger.info(f"Remaining retries: {self.state.judge_retries}")
							# update our task to include the original task, and failure_reason							
							self.task = f"Original Task: {self.original_task}\n\nWhat Failed: {judgement.failure_reason}" 

							# need to alter the is_done flag in our history to no longer be done
							# Our last item in the history should contain a result with the is_done flag set to true given
							# the initial if condition. Reset this flag to false to trick the model
							step = 1
							self.logger.info(f"Reset Step: {step}/{max_steps}")

						else:
							self.logger.info("No more judgement retries remaining.")
							break
