...
class AgentSettings(BaseModel):
	...
	# === [PATCH] File upload whitelist ===
	# 檔案白名單（完整檔案路徑）
	available_file_paths: list[str] | None = None
	# 資料夾白名單（允許該資料夾底下的所有檔案）
	available_dir_paths: list[str] | None = None
...
class AgentState(BaseModel):
	...
	judge_retries: int = 1 #a flag for determining how many times the judge should re-launch the agent
...
class AgentHistoryList(BaseModel, Generic[AgentStructuredOutput]):
	...
	# === [PATCH] Utilities for file-path safety in history ===
	def filter_disallowed_attachments(self, settings: AgentSettings) -> None:
		"""
		移除 history 中不在白名單的 attachments / images（避免把本機路徑寫進紀錄）。
		就地修改，不影響執行流程。
		"""
		files = settings.available_file_paths or []
		dirs = settings.available_dir_paths or []
		for h in self.history:
			for r in h.result:
				if r.attachments:
					r.attachments = [p for p in r.attachments if UploadWhitelist.is_allowed(p, files, dirs)]
				if r.images:
					# 如果 image dict 有 'path' 就檢查，否則略過（有的只放 base64）
					new_images = []
					for img in r.images:
						p = img.get('path') or img.get('file') or img.get('file_path')
						if p is None or UploadWhitelist.is_allowed(str(p), files, dirs):
							new_images.append(img)
					r.images = new_images

	def mask_sensitive_paths_in_results(self, settings: AgentSettings) -> None:
		"""
		把結果中的實際檔案路徑用 <masked_path> 取代（若不在白名單）。
		用於想保留欄位但不顯示真實路徑的情境。
		"""
		files = settings.available_file_paths or []
		dirs = settings.available_dir_paths or []
		for h in self.history:
			for r in h.result:
				if r.attachments:
					r.attachments = [
						p if UploadWhitelist.is_allowed(p, files, dirs) else "<masked_path>"
						for p in r.attachments
					]
				if r.images:
					for img in r.images:
						for k in ('path', 'file', 'file_path'):
							if k in img and img[k] and not UploadWhitelist.is_allowed(str(img[k]), files, dirs):
								img[k] = "<masked_path>"
...
# === [PATCH] Upload whitelist helper ===
class UploadWhitelist:
	"""
	提供：
	- is_allowed(path, files, dirs) : 檢查路徑是否允許
	- guard_action_upload(action_dict, settings): 用在執行工具前，若是上傳動作就檢查路徑
	"""
	...
	@staticmethod
	def is_allowed(path: str, files: list[str], dirs: list[str]) -> bool:
		if not path:
			return False
		target = UploadWhitelist._norm(path)

		# 檔案白名單（完整匹配）
		norm_files = {UploadWhitelist._norm(f) for f in files}

		if target in norm_files:
			return True

		# 資料夾白名單（前綴匹配）
		norm_dirs = [UploadWhitelist._norm(d) for d in dirs if d]
		for d in norm_dirs:
			try:
				# Windows & POSIX 同用：以 Path 比較父子關係
				target_path = Path(target)
				d_path = Path(d)
				# 是同一路徑或 target 位於資料夾底下
				if target_path == d_path or d_path in target_path.parents:
					return True
			except Exception:
				# 退回字串前綴
	...
	@staticmethod
	def guard_action_upload(action: dict, settings: AgentSettings) -> None:
		"""
		在呼叫「上傳檔案」的工具前先檢查。
		你可以在 Controller/Tool Executor 執行 action 前統一呼叫這個方法。
		支援的 action 形狀（擇一存在即可）：
		  { "upload_file": {"path": "..."} }
		...
		# 找出 upload payload
		payload = None
		if 'upload_file' in action and isinstance(action['upload_file'], dict):
			payload = action['upload_file']
		elif 'upload' in action and isinstance(action['upload'], dict):
			payload = action['upload']

		if not payload:
			return

		candidate = payload.get('path') or payload.get('file_path') or payload.get('file')
		if candidate and not UploadWhitelist.is_allowed(str(candidate), files, dirs):
			# 與你遇到的錯誤一致，但更易懂
			raise ValueError(
				f"File path '{candidate}' is not available. "
				f"Allow it via AgentSettings.available_file_paths / available_dir_paths."
			)
